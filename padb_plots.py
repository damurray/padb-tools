...
def _build_env_distribution_html(df: pd.DataFrame, cfg: dict, title: str) -> str:
    ...
    constants = "\n".join([
        ...
        f"var DIST_FREQS_ARR={json.dumps([round(f, 3) for f in all_freqs_sorted])};",
...
function freqDistKeyDown(e,which){
  if(e.key==='Enter'){freqDistTxtChange(which);return;}
  if((e.key==='ArrowUp'||e.key==='ArrowDown')&&typeof DIST_FREQS_ARR!=='undefined'&&DIST_FREQS_ARR.length){
    e.preventDefault();
    var txt=document.getElementById('dist_freq_'+which+'_txt');
    var slider=document.getElementById('dist_freq_'+which);
    if(!txt||!slider) return;
    var cur=parseFloat(txt.value);
    if(isNaN(cur)) cur=parseFloat(slider.value);
    var arr=DIST_FREQS_ARR;
    var idx=0,best=Infinity;
    for(var i=0;i<arr.length;i++){var d=Math.abs(arr[i]-cur);if(d<best){best=d;idx=i;}}
    var nxt=e.key==='ArrowUp'?Math.min(idx+1,arr.length-1):Math.max(idx-1,0);
    var v=arr[nxt];
    if(which==='lo'){var h=parseFloat(document.getElementById('dist_freq_hi').value);if(v>h)return;}
    else{var l=parseFloat(document.getElementById('dist_freq_lo').value);if(v<l)return;}
  ...
  conds.forEach(function(cd,ci){
    ...
    sorted.forEach(function(fs){
      ...
      hover.push(
        ...
        ((r.tll_lo!==null||r.tll_up!==null)?'TLL: ['+(r.tll_lo!==null?r.tll_lo.toFixed(4):'—')+', '+(r.tll_up!==null?r.tll_up.toFixed(4):'—')+']<br>':'')+
...
function buildBoxTraces(selConds,selTemps,yFlt,selBoxSers){
  ...
  BOX_DATA.forEach(function(cd){var _gk=getGroupKey(cd);if(condIdxMap[_gk]===undefined) condIdxMap[_gk]=ci++;});
  var _legendShown={};
  BOX_DATA.forEach(function(cd){
    if(selConds.indexOf(cd.condition)<0) return;
    if(selTemps.indexOf(cd.temp)<0) return;
...
/* ---- Site Population Check: for a cross-site (compare_csv) page, does a
   non-primary-site DUT's value at each frequency/temperature fall inside
   the k×IQR fence built from the primary site's own population at that
   same frequency/temperature? Reuses computeBoxStats/getIqrK -- the exact
   same fence already used for this view's own within-dataset outlier
   detection -- rather than a separate statistical method. */
function toggleSitePanel(){
  var panel=document.getElementById('box_site_panel');
  var btn=document.getElementById('box_site_toggle_btn');
  if(!panel||!btn) return;
  var show=panel.style.display==='none';
  panel.style.display=show?'':'none';
  btn.textContent=(show?'▼':'▸')+' Site Population Check';
...
/* Which direction is "toward failing" for this measurement, if determinable.
   ...
   override) rather than reading SPEC_DIRECTION directly -- 'hi' means an
   upper-limit-only spec (higher values are worse), 'lo' means lower-limit-
   only (lower values are worse). 'both'/'none' return null: with a two-sided
   spec neither direction is unambiguously "the bad one", so this doesn't
   editorialize when it can't tell. */
function _siteTowardFailDir(){
...
/* Suggested (not asserted) triage tag for one DUT's rollup -- see CLAUDE.md
   "Site Population Check triage hierarchy" for the reasoning behind each
   rule. Order matters: "shared across DUTs" is checked first because a
   station/systemic issue is the most consequential misread if missed
   (it taints every DUT from that site, not just one). */
function _siteTriageTag(d, towardFail){
  if(!d.outside) return null;
  var mostlyShared=d.sharedCount/d.outside>0.5;
  var mostlyHigh=d.high/d.outside>0.5;
...
var _lastSiteRows=[];
var _lastSiteMeta={};
/* Flags when a fence's own Q1/Q3/IQR population looks compromised, and
   names a specific fix -- two independent signals, not the same problem:
   (1) multiple conditions pooled into one fence, or genuine duplicate
       points from the same DUT -- inflates n with population(s) that
       don't belong together (a fence spanning hundreds of dB with n=60
       turned out to be every SpurType pooled together, discovered by a
       user reading the raw table). Fix: narrow to one condition, or
       Collapse dup runs. A duplicate here means the SAME (serial,
       condition, port) appears twice -- matching _dupBreakdown's own
       identity key -- NOT just the same serial, since one DUT legitimately
       appearing once under each of several pooled conditions is exactly
       what signal (1) already reports, not a duplicate measurement.
   (2) n and condition scope both look clean, but the fence's own
       reference population still contains a bad unit. Reuses
       computeBoxStats' own k*IQR outlier test on this exact population --
       deliberately NOT a "remove the worst point and see how much the
       fence shrinks" comparison: an IQR fence is intentionally robust to
       a single extreme value (Q1/Q3 barely move when only one of many
       points is removed), so that comparison rarely fires even on a
       textbook one-bad-unit case -- confirmed by hand-checking a synthetic
       10-clean-plus-1-extreme-outlier case, where a span-shrink test
       stayed under 15% while the point was obviously, individually
       outside its own population's fence. Whether any of the fence's own
       inputs is itself flagged an outlier by the same rule already used
       everywhere else in this view is the direct, already-proven test. */
function _fenceQualityHint(pvItems,k){
  if(!pvItems||pvItems.length<4) return '';
  var hints=[];
  var condSet={};pvItems.forEach(function(d){condSet[d.cond]=true;});
  var condCount=Object.keys(condSet).length;
  if(condCount>1){
    hints.push(condCount+' conditions pooled into this fence -- narrow to one condition for a cleaner comparison');
  }
  var idCounts={};
  pvItems.forEach(function(d){
    var key=d.s+'|'+d.cond+'|'+(d.p||'');
    idCounts[key]=(idCounts[key]||0)+1;
  });
  var dupExtra=0;
  Object.keys(idCounts).forEach(function(key){if(idCounts[key]>1) dupExtra+=idCounts[key]-1;});
  if(dupExtra>0){
    hints.push(dupExtra+' repeat point(s) from the same DUT -- consider Collapse dup runs');
  }
  var vals=pvItems.map(function(d){return d.v;});
  var full=computeBoxStats(vals,k);
  if(full.outliers&&full.outliers.length){
    var flagged=pvItems.filter(function(d){return full.outliers.indexOf(d.v)>=0;})
      .map(function(d){return d.s+'='+d.v.toFixed(2);});
    hints.push(full.outliers.length+' point(s) in this fence\'s own population are themselves outliers ('+
      flagged.join(', ')+') -- consider excluding via the Global Filter');
  }
  return hints.join(' | ');
}
function updateSitePanel(){
  ...
  try{
    ...
    var rows=otherPoints.map(function(p){
      ...
      var fenceHint=_fenceQualityHint(pvItems,k);
      /* A one-sided spec can only be failed by moving toward the bad side --
         e.g. an upper-limit-only spec (towardFail='high') can never be
         failed by reading LOW, so a fence-lo/Dir=low OUTSIDE there is a
         population difference, not a compliance concern. null (two-sided or
    ...
    /* Per-frequency clusters: how many distinct DUTs are OUTSIDE at the
       exact same (site,temp,freq) -- multiple independent DUTs failing the
       same spot points at the station/fixture/calibration, not any one
       DUT, and is arguably more urgent than a single bad DUT since it
       taints every DUT measured there. */
    var freqMap={};
    rows.forEach(function(r){
      if(r.verdict!=='OUTSIDE') return;
      var key=r.p.site+'|'+r.p.temp+'|'+r.p.freq;
      var e=freqMap[key]||(freqMap[key]={serials:new Set(),freqLabel:r.p.freqLabel||r.p.freq,site:r.p.site,temp:r.p.temp});
      e.serials.add(r.p.serial);
    });

    /* Per-DUT rollup: breadth (how many freqs affected) and direction split
       (toward-fail vs away-from-fail) are what separate "likely bad DUT"
       from "likely benign" from "likely a shared station issue". */
    var dutMap={};
    rows.forEach(function(r){
      var key=r.p.site+'|'+r.p.serial;
      ...
      if(r.verdict==='OUTSIDE'){
        ...
        if(r.dir==='high') d.high++; else d.low++;
        if(r.dist>d.maxDist) d.maxDist=r.dist;
        d.freqKeys.push(r.p.temp+'|'+r.p.freq);
      }
    });
    Object.keys(dutMap).forEach(function(key){
      var d=dutMap[key];
      var shared=0;
      d.freqKeys.forEach(function(tf){
        var fk=freqMap[d.site+'|'+tf];
        if(fk&&fk.serials.size>1) shared++;
      });
      d.sharedCount=shared;
    ...
    var outsideRows=rows.filter(function(r){return r.verdict==='OUTSIDE';});
    var nOutside=outsideRows.length;
    var nBenign=outsideRows.filter(function(r){return r.specRelevant===false;}).length;
    var nNA=rows.filter(function(r){return r.verdict==='n/a';}).length;
    ...
    var html='<div style="font-size:12px;margin-bottom:6px"><b>'+nOutside+'</b> of <b>'+rows.length+
      ...
      (nBenign?' (<b>'+nBenign+'</b> of those are benign -- away from the spec-fail direction)':'')+
    ...
    if(freqClusters.length){
      html+='<div style="font-weight:600;margin:8px 0 2px">Frequency clusters -- multiple DUTs affected at the same spot (points at a station/fixture/calibration issue, not a single DUT)</div>';
      html+='<table class="stbl"><thead><tr><th>Site</th><th>Temp</th><th>Freq</th><th>DUTs affected</th><th>Serials</th></tr></thead><tbody>';
      freqClusters.forEach(function(e){
        html+='<tr><td>'+e.site+'</td><td>'+e.temp+'</td><td>'+e.freqLabel+'</td><td class="sev-hi">'+e.serials.size+'</td>'+
          '<td>'+Array.from(e.serials).join(', ')+'</td></tr>';
      });
      html+='</tbody></table>';
    } else {
    ...
    rows.forEach(function(r){
      ...
      var vTd=r.verdict!=='OUTSIDE'
        ?(r.verdict==='n/a'?'<td style="color:#aaa">n/a</td>':'<td>inside</td>')
        :r.specRelevant===false
          ?'<td style="background:#eef3fb;border-left:2px solid #7a9cc6;color:#2c5c96" title="Away from the spec-fail direction -- population difference only, cannot fail this spec.">OUTSIDE (benign)</td>'
          :'<td style="background:#fff0e8;border-left:2px solid #e0905a;color:#c04000;font-weight:bold">OUTSIDE</td>';
...
/* ---- Auto-filter bad DUTs (selectable basis x level) ----
   Adds clearly-bad DUTs' points to the Global Filter, tiered by a confidence
   LEVEL and a BASIS (what "bad" means). Magnitude is unified to sigma so the
   level bar means the same across bases: Distribution = |sigma| from the box's
   own peers (mean/std); Spec/TLL = how many sigma a fail-side point sits PAST
   the limit (spec, or the guard-banded TLL/override). Systemic (one frequency,
   >1 DUT) and benign (away from a one-sided spec) are never auto -- always left
   for the user. Nothing is applied until Apply; every DUT carries a reason. */
var _AUTO_LEVELS={
  conservative:{minSigma:6,minPts:3,label:'Conservative'},
  moderate:{minSigma:4,minPts:2,label:'Moderate'},
  aggressive:{minSigma:3,minPts:1,label:'Aggressive'}
};
function _autoLevel(){var e=document.getElementById('auto_gf_level');return e?e.value:'off';}
function _autoBasis(){var e=document.getElementById('auto_gf_basis');return e?e.value:'dist';}
function _autoSiteOf(cond){var m=String(cond||'').match(/Site:\s*([^|]+?)(?:\s{2,}|$)/i);return m?m[1].trim():'';}
function _autoMedian(a){var s=a.slice().sort(function(x,y){return x-y;});var n=s.length;return n?(n%2?s[(n-1)/2]:0.5*(s[n/2-1]+s[n/2])):0;}
/* Robust badness magnitude via the MEDIAN and MAD (median absolute deviation),
   NOT mean/std. Mean/std saturate for small per-box n -- with 10 points a single
   gross outlier can't exceed ~2.85 sigma because it inflates the very std in its
   own denominator, so a sigma bar of 3-6 is unreachable and two bad units look
   "systemic". median/MAD have a 50% breakdown point: the median and MAD are set
   by the clean majority, so a bad point's modified z-score (0.6745*(x-med)/MAD,
   Iglewicz-Hoaglin) grows without bound with how bad it is. Distribution basis
   flags points past the 3.5 modZ cutoff; Spec/TLL basis measures fail-side
   exceedance in the same MAD-scaled units, so the level bar means the same thing. */
function _autoBadPoints(basis){
  var selConds=getSelectedConds(),selTemps=getSelectedTemps(),yFlt=getYFilter(),selBoxSers=getSelectedBoxSerials();
  var allSers=getAllBoxSerials();
  var serActive=selBoxSers&&allSers.length>1&&selBoxSers.length<allSers.length;
  var fr=getBoxFreqRange();
  var hiOv=(yFlt&&yFlt.tll_hi!==null&&yFlt.tll_hi!==undefined)?yFlt.tll_hi:null;
  var loOv=(yFlt&&yFlt.tll_lo!==null&&yFlt.tll_lo!==undefined)?yFlt.tll_lo:null;
  var out=[];
  BOX_DATA.forEach(function(cd){
    if(selConds.indexOf(cd.condition)<0)return; if(selTemps.indexOf(cd.temp)<0)return;
    (cd.freq_stats||[]).forEach(function(f){
      if(f.freq<fr.lo||f.freq>fr.hi)return;
...
/* Normal survival function (one-sided tail) via an erf approximation
   (Abramowitz & Stegun 7.1.26) -- for the risk metric below. */
function _normSf(z){ z=Math.abs(z);
  var t=1/(1+0.3275911*z);
  var erf=1-(((((1.061405429*t-1.453152027)*t)+1.421413741)*t-0.284496736)*t+0.254829592)*t*Math.exp(-z*z);
  return 0.5*(1-erf);   // P(X > z) for standard normal
}
/* Per-DUT false-removal RISK: probability that a genuinely in-family unit would,
   by chance under a normal model, read as extreme as this DUT's worst point.
   Distribution basis is two-sided (outlier either way); Spec/TLL is one-sided
   (only the fail direction). Bonferroni-scaled by the number of points examined
   so "worst of many" isn't mistaken for a single-draw tail -- a conservative
   guard against auto-removing a DUT that's only extreme because we looked at a
   lot of points. Lower risk = more confident the removal is real. */
function _autoRisk(maxMag,basis,nExamined){
  var pOne=_normSf(maxMag);
  var pPoint=(basis==='dist')?2*pOne:pOne;
  var p=1-Math.pow(1-Math.min(1,pPoint),Math.max(1,nExamined||1));
  return Math.min(1,p);
}
...
function autoFilterPreview(){
  ...
  h+='<div style="font-weight:600;margin:8px 0 2px;color:#6b5a00">Marginal — review &amp; affirm: '+r.marginal.length+
    (r.marginal.length?' &nbsp;<button class="toggle-btn" style="background:#fff8e1;border-color:#e0c05a;color:#6b5a00" onclick="autoFilterAffirm()">Also filter checked</button>':'')+'</div>';
  if(r.marginal.length) h+='<table class="stbl"><thead><tr><th></th><th>Serial</th><th>Pts</th><th>Max</th><th>Risk</th><th>Reason</th></tr></thead><tbody>'+
    r.marginal.map(function(d,i){return '<tr><td><input type="checkbox" class="auto_marg_chk" data-i="'+i+'"></td><td>'+d.serial+'</td><td>'+d.pts.length+'</td><td>'+d.maxMag.toFixed(1)+'</td><td title="p≈'+d.risk.toExponential(1)+'">'+_riskLabel(d.risk)+'</td><td style="white-space:normal;max-width:480px">'+d.reason+'</td></tr>';}).join('')+'</tbody></table>';
  if(r.review.length){
    h+='<div style="font-weight:600;margin:8px 0 2px;color:#2c5c96">Left for you — never auto-filtered: '+r.review.length+'</div>';
    h+='<table class="stbl"><thead><tr><th>Serial</th><th>Pts</th><th>Reason</th></tr></thead><tbody>'+
      r.review.map(function(d){return '<tr><td>'+d.serial+'</td><td>'+d.pts.length+'</td><td style="white-space:normal;max-width:560px">'+d.reason+'</td></tr>';}).join('')+'</tbody></table>';
...
function autoFilterAffirm(){
  var r=window._autoResult; if(!r)return;
  var keys=[];
  document.querySelectorAll('.auto_marg_chk:checked').forEach(function(c){
    var d=r.marginal[parseInt(c.getAttribute('data-i'),10)]; if(d)keys=keys.concat(d.keys);
  });
  if(!keys.length){alert('No marginal DUTs checked.');return;}
...
def _build_box_interactive_html(
    ...
    if primary_site:
        site_btn_html = (
            '  <button class="toggle-btn" id="box_site_toggle_btn"'
            ' title="Tests each non-primary-site DUT\'s value at each frequency/temperature'
            ' against the k&times;IQR fence built from the primary site\'s (' + primary_site + ')'
            ' own population at that frequency/temperature -- uses the k&times;IQR control above"'
            ' onclick="toggleSitePanel()">&#9658; Site Population Check</button>\n'
        )
...
def _stat_boxplot_interactive(csv_path: Path, cfg: dict, output_html: Path) -> None:
    ...
    # Cross-site comparison (compare_csv in job.json tags each row's Group
    # text "Site: <name>" before this function ever sees it -- see padb_v2.py
    # _build_compare_csv). Extracted independently of cond_keys/COND_DIMS so
    # a Site with only 1 distinct value (or excluded by some other rule)
    # still resolves correctly here.
    _site_re_box = re.compile(r'Site:\s*(\S+)')
    if "Group" in df.columns:
        df["_site"] = df["Group"].map(
            lambda g: (_site_re_box.search(g).group(1) if pd.notna(g) and _site_re_box.search(g) else None)
        )
    else:
        df["_site"] = None
    all_sites = sorted(df["_site"].dropna().unique().tolist())
    primary_site = cfg.get("primary_site")
    site_compare_enabled = bool(primary_site) and primary_site in all_sites and len(all_sites) > 1
    coverage_gap_html = ""
    if site_compare_enabled:
        def _norm_val(v):
            # Different sites' own PADB extractions can format the identical
            # numeric spec/limit/uncertainty value with different trailing
            # precision (confirmed real: one real pod's SR CSV writes
            # "Upper Spec (<=): -100.00", its AMC2 CSV writes "-100" for the
            # same value) -- comparing raw strings across sites would report
            # that as a false coverage gap. Normalize anything numeric-
            # looking before diffing; non-numeric values (e.g. SpurType
            # names) compare as plain strings, unaffected.
            try:
                return round(float(v), 6)
            except (TypeError, ValueError):
                return v
        ...
        def _site_coverage_gaps(dim_label: str, series) -> list:
            tmp = pd.DataFrame({"_site": df["_site"], "_v": series})
            ...
            # One display string per normalized value -- doesn't matter which
            # site's own formatting it borrows, since post-normalization
        ...
        if (df["_port"] != "").any():
            _gap_lines += _site_coverage_gaps("Port", df["_port"].where(df["_port"] != ""))
    ...
    if site_compare_enabled:
        for cd in box_data:
            ...
            cd["site"] = m.group(1) if m else None
    stat_data_box = _aggregate_stat_data(df, cfg)
