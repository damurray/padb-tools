...
**Spec-limit segment tab-through** (added 2026-08-06): a "Segment by" selector (Spec / Limit / Uncertainty) plus Prev/Next buttons let you jump the frequency range directly to each contiguous band of a frequency-varying spec. **As of 2026-08-21, the whole control is omitted from the page (not rendered at all) if none of Spec/Limit/Uncertainty has more than one distinct value anywhere in the dataset** — a genuinely flat/constant-spec pod simply won't show a "Segment by" control. If the control *is* present, **it only works fully if the pod's extraction was configured to include the `Upper Spec`/`Lower Spec` and/or `Upper Uncertainty`/`Lower Uncertainty` grouping items** — most existing pods only have the default `Upper Limit`/`Lower Limit`, which is spec adjusted per-unit by measurement uncertainty and is *not* reliably frequency-piecewise-constant (it fragments into more, noisier segments, or fails to reveal band structure at all). To add these: open the pod in PADB-R.exe, add `Upper Spec`/`Lower Spec` (and optionally `Upper Uncertainty`/`Lower Uncertainty`) as extraction grouping items on the Type=80 analytic, and re-save. Without them, "Segment by: Spec/Uncertainty" will show zero segments and the Prev/Next bar stays hidden (the selector itself still shows, since `Upper_Limit`/`Lower_Limit` still has real variation) — falls back to "Limit" automatically if that's the only one with data.
...
**Related, but distinct, bug fixed 2026-08-20**: a stale R-Plots leftover (typically from a manual PADB-R GUI session, which does default-save there) sharing a stem with an analytic that *does* have genuinely fresh `-dir` output this run used to silently overwrite that fresh output with the old one — a real, confirmed incident, not a theoretical gap. See `CLAUDE.md` → **`_collect_padb_outputs()` clears stale files before copying fresh ones** for the fix. This is the opposite direction from the case just above (that one is about a stem with *no* fresh output at all; this one is about a stem that *does* have fresh output, being clobbered anyway).

**The "no fresh output at all" case is now closed too (changed 2026-08-27): `_collect_padb_outputs()` no longer scans R-Plots at all.** An analytic with no fresh `-dir` output this run used to fall through to whatever R-Plots happened to have under a matching filename, right or wrong — R-Plots is PADB's shared default save location, so any concurrent or prior manual session, or an unrelated job/site, can write there too, with nothing to tell "this run's genuine output" apart from "someone else's leftover" beyond a filename match. A real incident: a manual AMC2 GUI session's R-Plots leftover got silently copied into an SR job's own results under the identical un-suffixed filename, and a site-comparison job ended up comparing AMC2 against itself. `results_padb` is now the only trusted source, full stop — a genuinely missing analytic is reported by name as an unresolved gap (pod config, date range, `Filter_Expression` zeroing out every row, or a data type that never supported `-dir`) instead of being silently patched from a shared folder. The one legitimate historical use of R-Plots — a CSV manually placed into `results_padb` once, for output PADB never writes via any path at all (the clock-spurs SummaryPlot gotcha, see `CLAUDE.md`) — is unaffected, since this function never deletes an existing `results_padb` file.
