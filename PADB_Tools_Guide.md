...
**Web app "Clean up orphaned PADB-R" button** (Running Jobs section, added 2026-08-21) is the point-and-click alternative to the manual `taskkill` above — it lists every batch-invoked (`-f <switchfile>`) PADB-R.exe running system-wide (never an idle GUI window someone has open by hand), shows exactly what it found, warns if this browser session has an active job, and kills whichever PIDs you confirm — also taking down each one's orphaned `R-Host.exe` helper process (a separate PADB-R.NET resource leak: this helper binary doesn't clean up after itself when its parent PADB-R.exe exits).
