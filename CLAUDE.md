...
Local Flask app (opens `http://127.0.0.1:5000` in the default browser). Local use only — the dev server is not meant to be reachable beyond 127.0.0.1. Every route shells out to the existing CLI scripts via `subprocess`, or imports their pure functions directly (`parse_pod_analytics()`, `discover_all_padb_tasks()`, `create_task()`, `delete_task()`, `query_task()`, `format_schedule_summary()`); nothing in `padb_run.py`/`padb_v2.py`/`padb_make_job.py`/`padb_make_v2_job.py` was changed to build this (one real bug in `padb_scheduler.py` itself was found and fixed along the way — see below).

Phase 1 covers three of the seven originally-requested features:
...
Deliberately out of scope for Phase 1 (not started): site conversion via the UI, the guided "what do you want to do?" wizard, the tutorial walkthrough dropdown, and the doc viewer.
