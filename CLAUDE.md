...
All 6 V2 views have a "Segment by" selector (Spec / Limit / Uncertainty) plus Prev/Next buttons that jump the frequency range to each contiguous band of a frequency-varying spec — e.g. a datasheet spec that steps from -100 dBc to -94 dBc to -88 dBc as frequency increases: `accuracy_vs_freq`/`render_scatter` (scatter), `distribution`, `stat_boxplot` (boxplot), `stat_summary`, `render_summary` (summary, padb_v2.py), and `render_env_coverage` (env_coverage, padb_v2.py).
...
- **Boxplot, stat_summary, summary, and env_coverage are all architecturally different** from scatter/distribution — each pre-aggregates server-side into its own distinct per-condition shape, not a flat per-row array, and each needed its own (structurally similar but not identical) segment-detection function as a result:
  ...
  - None of these four had pre-existing per-DUT-per-frequency Limit/Spec/Uncertainty tracking before this work — scatter/distribution/boxplot's `Spec_Hi`/`Spec_Lo`/`Unc_Hi`/`Unc_Lo` CSV columns (`_load_scatter_csv`/`_load_scatter_for_stats`) already existed from the Spec/Uncertainty parsing described above; the aggregation functions just hadn't threaded them through to their client-side JSON yet.
