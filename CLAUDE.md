...
- **Re-import: an exported CSV is re-plottable through `histogram()`** (added 2026-09-09). The export now also carries `Upper Limit`/`Lower Limit` columns (empty when unset), and `_load_histogram_csv()` handles the "flat" export shape — when there's **no `Group` column**, it synthesizes one from the separate dim columns (each non-value/non-limit/non-serial/non-"Out of spec"/non-metadata column) plus the `Serial` column, so the normal Group parser reproduces the same dims + serial. So an Export → (optionally edit in Excel: drop DUTs, merge runs) → re-plot round-trips: re-run any histogram job with `--csv <exported.csv>` (it reuses `views:["histogram"]`), e.g. `py padb_v2.py SwitchingSpeed_OA_Switching_v2_job.json --csv myexport.csv --out <dir> --no-publish`. Verified a full round-trip: native (dims=[Port], 11,200 values, 8 serials, hi=11000, 1,553 fails) → flat export → re-load reproduces every field exactly, spec line redrawn, pass/fail recomputed to the same 1,553.
