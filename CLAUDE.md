...
**Keep the self-contained HTML for normal small/medium analytics** — it works great and needs nothing. The viewer + parquet are only for the giants. **Next steps:** add the other views (boxplot/summary/stat_summary/env_coverage/distribution) the same server-side-query way; PyInstaller → single `.exe` so no-Python users double-click a viewer in the results folder.
