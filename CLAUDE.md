...
- **Wired into boxplot + stat_summary** (prototype-first, per the established pattern): `BOX_AF.subpopSlices()` yields one slice per (condition, temp) currently shown; `STAT_AF.subpopSlices()` one per active condition (Room-only) — per-DUT mean per frequency from `vals_detail`/`dut_vals` (base serial; repeats collapse/pre-averaged), budget = median `|unc_hi|` per freq when present (else shape-only), respecting the condition/temp/serial filters. **Not yet:** GF-parity in `subpopSlices` (a refinement), station/port correlation (no station field embedded; passed `null`), and the remaining three views (`summary`/`env_coverage`/`histogram` — each adds its own `subpopSlices`).
