...
- **Wired into boxplot + stat_summary + summary** (prototype-first, per the established pattern): `BOX_AF.subpopSlices()` yields one slice per (condition, temp) shown; `STAT_AF.subpopSlices()` one per active condition (Room-only); `SUM_AF.subpopSlices()` one per active condition (temp-blind, per-DUT value = `dut_vals[fi][di]`, budget from `dut_spec_vals.unc_hi`) — per-DUT mean per frequency, base serial, budget = median `|unc_hi|` per freq when present (else shape-only), respecting the condition/temp/serial filters. **Not yet:** GF-parity in `subpopSlices` (a refinement), station/port correlation (no station field embedded; passed `null`), and the remaining two views (`env_coverage`/`histogram` — each adds its own `subpopSlices`).
