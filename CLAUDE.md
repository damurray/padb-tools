...
- **`padb_bands.py`** — shared named-band config (one source of truth for the viewer + HTML views): `load_bands_file` (unit-convert to the data x-unit), `auto_bands` (derive an editable starter from the swept x), `write_auto_bands`, `find_or_create_bands` (sidecar-wins-else-auto-generate-once). See **Named-band segment step**.
