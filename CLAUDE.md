...
- **TODO — Config-driven coverage set for `qa_view_sweep.py`** (requested 2026-09-03). The QA view-sweep's `_DEFAULT_JOBS` coverage list (one representative job per pod-variety axis) is hardcoded with *this workstation's* pod-name globs (ClockSpurs, Close-In, MaxPowerTutorial, …), so another group's sweep would mostly `[SKIP]` unless they use `--no-defaults --job …` or edit the script. Make it config-driven: read the coverage list from a per-user `qa_view_sweep.json` manifest (same per-user pattern as `padb_config.json`) — each entry `{label, glob(s), needles?}` — falling back to the built-in list only when no manifest exists. That lets each group maintain their own coverage set (their pod names + expected needles) without touching the script. Small change; `--root`/`--job`/`--no-defaults` already make the *mechanism* portable — this is just about the default coverage list. (The generic checkers `padb_csv_check.py`/`qa_padb.py`/`qa_js_segments.py` and `qa_csv_sweep.py --root` are already fully portable; the core run-job/plot pipeline is path-agnostic via job.json + per-user `padb_config.json`.)
- **TODO — Live "Show all points" toggle on the scatter view** (requested 2026-09-03). Today `scatter_decimate` is a *build-time* server-side reduction (`_decimate_dense_series`): when on, only the min/max-envelope points are embedded in the HTML, so the dropped points are gone from the page and can't be recovered without re-running the plot job (`padb_v2.py`, no re-extraction needed). A viewer doing a critical review has to flip `"scatter_decimate": "off"` in job.json and rebuild to see every dot. The ask: a client-side checkbox that switches the scatter between the decimated (envelope) rendering and all points, live, without a rebuild. **Key tradeoff to design around:** decimation's *primary* benefit is shrinking the embedded data (file size); a live toggle requires the **full** point set to be embedded, which negates the size saving. So this is only useful for the "page size is acceptable, but I want faster default *rendering* with an on-demand full view" case — not for the giant-file case decimation exists to fix. Cleanest implementation: an opt-in job.json key (e.g. `"scatter_decimate_toggle": true`) that embeds the full series and runs the identical min/max-envelope bucketing (a JS port of `_decimate_dense_series`) *client-side*, so only one data copy is embedded and the checkbox just chooses which to draw. Make it clearly distinct from the existing build-time `scatter_decimate` (which stays the default for size control). Confirm the intended use case with the user before building — if it's really "always want all points," `scatter_decimate: "off"` already does that at build time with no new feature.
- **TODO — Extend Site Population Check to `env_coverage` and `distribution`** (plot-type-review item 4, deferred 2026-09-03 at user's request; involved, not started). The cross-site fence-membership panel currently lives only in `boxplot`/`stat_summary`/`summary`. Template to copy: `_STAT_SUMMARY_JS`'s `toggleSitePanel`/`updateSitePanel`/`_siteFence`/`_siteTriageTag`/`_siteTowardFailDir`/`saveSitePopulationCSV` (~lines 6012–6293) plus the server-side gating (`site_compare_enabled`/`coverage_gap_html`/`PRIMARY_SITE` constant/`site_btn_html`, ~7005–7075 and ~7427–7440). Only `updateSitePanel()`'s point-gathering is per-view; the helpers are view-agnostic. Notes when picked up: (a) **distribution** is the natural fit — per `(temp, freq)` fence on the abs values in `RAW_ABS` (like boxplot); the per-point raw Group is already available as `.g` (added for GF 2026-09-02), so site can be parsed from it (`Site: <name>`) rather than adding a new per-point field; neither `_build_env_distribution_html` nor `_build_env_coverage_html` currently extracts `_site`/`PRIMARY_SITE` or a coverage-gap banner, so that plumbing is new. (b) **env_coverage** has a genuine semantic choice to confirm with the user first — its data is ΔEnv (temperature deltas), not raw values, so the sensible "population" to fence on is the per-DUT **Room** value per frequency (Room-only, mirroring stat_summary), not the deltas. (c) The real Close-In compare dataset has no serials, so the per-DUT/frequency-cluster triage degrades to a plain OUTSIDE count there — verify against a with-serial compare (e.g. `MaxPowerTutorial2` SR-vs-AMC2) for the full triage.
