...
- Only wired into `boxplot`. The other 5 V2 views will still render fine against a `compare_csv`-merged dataset (Site is just a condition dimension to them too), but none of them have a dedicated site-comparison feature yet — `stat_summary` would be the next natural candidate if this proves useful, since it already has the closest analogous per-condition statistics.
