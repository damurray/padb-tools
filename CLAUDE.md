...
- **Wired into boxplot only so far** (prototype-first, per the established pattern): `BOX_AF.subpopSlices()` yields one slice per (condition, temp) currently shown — per-DUT mean per frequency from `vals_detail` (base serial; repeat runs collapse), budget = median `|unc_hi|` per freq when present (else shape-only), respecting the condition/temp/serial filters. **Not yet:** GF-parity in `subpopSlices` (a refinement), station correlation for boxplot (no station field in `BOX_DATA`), and the other four views (`stat_summary`/`summary`/`env_coverage`/`histogram` — each would add its own `subpopSlices`).
