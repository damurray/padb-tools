...
- **Busy spinner on table/panel render + Site-check row cap** — the Site Population Check panels paint a "Computing Site Population Check…" spinner and build on the next frame (so a heavy panel shows progress instead of freezing), and their per-point detail tables cap at 5,000 rows (the summary counts and CSV export still cover all rows). **Scope note:** the render spinner covers the *Site Population Check panels* — the previously-unguarded slow tables (e.g. a 62-dimension switching-speed compare with ~12k+ non-primary rows). The main **Statistics/Results tables** are deliberately *not* spinner-wrapped: they already have a size-gate (auto-refresh below ~150 conditions, a highlighted manual "Refresh table" above it) that prevents the same freeze.
