...
- **Sep 3** — Boxplot "Group by" is now a **multi-select**: pool the boxes by any *combination* of parameters (Ctrl/Cmd-click), not just one dimension or all. Selecting none = Condition (no pooling); a subset pools across the parameters you didn't pick. Legend and Statistics Table reflect the combination. (Boxplot first; the other views' Group by will follow.)
