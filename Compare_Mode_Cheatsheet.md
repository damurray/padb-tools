...
- Long CSV lists: use the **filter box** to narrow both dropdowns by name (same idea as the Jobs table's Name filter), and **Refresh CSVs** to re-scan disk without reloading the page (e.g. after a new extraction lands). The dropdowns and Description field are wide enough for long file names.
