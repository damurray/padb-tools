...
- **Segment by** *(with Prev / Next buttons)* — for data with a spec that changes at different frequency bands (a "staircase" spec), this jumps the frequency range straight to each band in turn. **The whole control is hidden, not just Prev/Next, when the plot's data has no frequency-varying Spec/Limit/Uncertainty at all** — if you don't see a "Segment by" control, that's expected for a flat/constant spec, not a bug. If the control *is* there but Prev/Next don't do anything, see the troubleshooting section below. *(Present on Scatter, Boxplot, Stat Summary, Summary, Env Coverage, and Distribution, whenever the underlying data has something to segment by.)*
  - **Named bands** — if a band file (`bands.json` / `padb_viewer_bands.json`) sits next to the results, "Segment by" also offers a **Named bands** mode that steps the frequency window through *your* named bands (e.g. DAC / Low / Mid / High) instead of the spec staircase — the band name shows in the label, and stepping narrows the plot **and** the tables to that band just like the spec stepper. Set one up with **`py padb_make_bands.py <results-folder>`** (auto-generate an editable starter from the data), `--band "Name:lo:hi"` (type your own), or `--sg6311a` (standard preset); then edit the file to rename/re-range. This is opt-in — no band file, no Named-bands option. (A band file is *not* limited to RF frequency; it just breaks up whatever the swept x-axis is.)
...
- **Distribution of table data** *(Boxplot, Per-point table mode only)* — a button that opens a histogram of the values in the per-point table, overlaid by your current *Group by* (with mean/median and spec/limit lines), plus a summary-stats table (n, mean, median, σ, min/max, p5/p95, # fail / n). It honors every current filter and uses **all** the filtered points, not just the first 5000 the table displays. Tick **"KDE overlay + modality hint"** to add a smoothed density curve per group and a *modality* readout — the KDE peak count and a bimodality coefficient (BC > 0.555 leans bimodal) — which helps spot two separate populations. It's an advisory screen, not proof: confirm with the Subpopulation advisory in *Workflow & Recommendations*. **Important:** if you leave it pooled across many frequencies it will warn you — the shape then just reflects how the level changes with frequency, not real sub-populations, so narrow to one frequency (the freq range or a Segment step) for a true single-population / bimodality read.
...
- **🔒 Locked filters** — a small floating panel (top-right of every view) that saves the *common* filters so you don't have to dial them in again on each page. Set up your condition filters (harmonic, mode, site, serial, port, temperature…), frequency range, and All/Passing/Failing on one view, then click **"🔒 Lock these filters."** Every other view **auto-applies the same set when you open it**, and the panel shows what's locked with **Apply** (re-apply here), **Update from this view** (overwrite the lock with this page's current filters), **Clear**, and **Export / Import** (save the lock to a file — use this to carry it to a page you open straight off the network share, where browser storage isn't shared). It's best-effort: a locked filter that doesn't exist on a given view is simply ignored (it never blanks a page), and things that only make sense on one view (group-by, spec segments, table mode, zoom) are deliberately *not* locked. Works from any view to any view.
