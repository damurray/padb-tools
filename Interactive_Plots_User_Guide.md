...
- **🔒 Locked filters** — a small floating panel (top-right of every view) that saves the *common* filters so you don't have to dial them in again on each page. Set up your condition filters (harmonic, mode, site, serial, port…), frequency range, and All/Passing/Failing on one view, then click **"🔒 Lock these filters."** Every other view **auto-applies the same set when you open it**, and the panel shows what's locked with **Apply** (re-apply here), **Update from this view** (overwrite the lock with this page's current filters), **Clear**, and **Export / Import** (save the lock to a file — use this to carry it to a page you open straight off the network share, where browser storage isn't shared). It's best-effort: a locked filter that doesn't exist on a given view is simply ignored (it never blanks a page), and things that only make sense on one view (group-by, spec segments, table mode, zoom) are deliberately *not* locked. Works from any view to any view.
